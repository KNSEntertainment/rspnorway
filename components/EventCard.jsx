...
const EventCard = ({ date, title, time, venue }) => {
	...
	return (
		<div className="relative flex flex-col justify-between bg-white/70 backdrop-blur-md border border-gray-200 shadow-xl rounded-2xl p-6 h-72 w-72 mx-auto transition-transform duration-300 hover:scale-105 hover:shadow-2xl group overflow-hidden">
			{/* Accent Bar */}
			<div className="absolute left-0 top-0 h-full w-2 bg-gradient-to-b from-[#0094da] to-[#0094da] rounded-l-2xl" />

			{/* Date Section */}
			<div className="flex items-center gap-4 mb-4 z-10">
				<div className="w-16 h-16 bg-gradient-to-br from-[#0094da] to-[#0094da] rounded-xl flex flex-col items-center justify-center shadow-lg border-4 border-white group-hover:scale-110 transition-transform">
				...
				<div className="flex-1">
					<h3 className="text-lg font-bold text-gray-800 group-hover:text-[#0094da] transition-colors line-clamp-2">{title}</h3>
				</div>
			</div>
			...
			<div className="flex-1 flex flex-col justify-end z-10">
				<div className="flex items-center gap-2 mb-2">
					<svg className="w-5 h-5 text-[#0094da]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
			...
			<div className="absolute -bottom-10 -right-10 w-32 h-32 bg-[#0094da] rounded-full blur-2xl opacity-60 z-0" />
		</div>
	);
};
