...
export default function MembershipBulkUpload({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const downloadTemplate = () => {
    const csvContent = `fullName,email,phone,address,city,postalCode,dateOfBirth,gender
...
Jane Smith,jane@example.com,+47 987 65 432,High Street 45,Bergen,5000,1985-05-15,female`;
  ...
  const validateCSV = (content: string): ValidationResult => {
    ...
    const requiredHeaders = ['fullname', 'email', 'phone', 'address', 'city', 'postalcode', 'dateofbirth', 'gender'];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!member.fullname) errors.push(`Row ${i + 1}: Full name is required`);
      ...
      if (member.fullname && member.email && member.phone && member.address && member.city && 
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('/api/membership/bulk-create', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg border-2 border-brand">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Bulk Upload Members</h2>
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <ul className="text-sm text-gray-700 space-y-1">
          <li>• <strong>fullName</strong> - Full name (required)</li>
        ...
        <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
          <p className="text-sm text-blue-700">
            <strong>Note:</strong> All members will be created with &quot;general&quot; membership type and &quot;pending&quot; status by default.
          </p>
        </div>
      </div>
    </div>
