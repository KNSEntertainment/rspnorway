...
export default function FooterClient({ settings, t }) {
	return (
		<footer className="bg-gradient-to-b from-gray-50 to-white text-gray-800 pt-12 pb-6">
			<div className="container mx-auto px-4">
				...
				<div className="flex flex-col items-center text-center mb-10 md:hidden">
					...
					<p className="text-sm text-gray-600 max-w-xs leading-relaxed">{t.tagline}</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-8">
					...
					<div className="hidden md:flex flex-col items-center justify-start text-center">
						...
						<p className="text-sm text-gray-600 max-w-md leading-relaxed">{t.tagline}</p>
					</div>
					...
					<div className="space-y-4">
						...
						<div className="space-y-3">
							...
							<div className="flex items-start gap-3 group">
								...
								<p className="text-sm text-gray-600 leading-relaxed">{t.address}</p>
							</div>
							...
							<a href={`mailto:${settings?.[0]?.email}`} className="flex items-center gap-3 group hover:translate-x-1 transition-transform">
								...
								<span className="text-sm text-gray-600 hover:text-brand transition-colors">{settings?.[0]?.email}</span>
							</a>
							...
							<a href={`tel:${t.phone}`} className="flex items-center gap-3 group hover:translate-x-1 transition-transform">
								...
								<span className="text-sm text-gray-600 hover:text-brand transition-colors">{t.phone}</span>
							</a>
				...
				<div className="border-t border-gray-200 pt-6 mt-8">
					...
					<div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-4">
						<nav className="flex flex-wrap gap-4 justify-center md:justify-start">
							<Link href="/en/terms-and-conditions" className="text-xs text-gray-500 hover:text-brand transition-colors">
								{t.terms}
							</Link>
							<span className="text-gray-300 hidden md:inline">•</span>
							<Link href="/en/privacy-policy" className="text-xs text-gray-500 hover:text-brand transition-colors">
								{t.privacy}
							</Link>
						</nav>

						<div className="text-xs text-gray-500 text-center md:text-right">{t.copyright || `© ${new Date().getFullYear()} RSP Norway. All rights reserved.`}</div>
					</div>
